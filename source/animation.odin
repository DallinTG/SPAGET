...
animations::struct{
	global:[Animation_Name]Animation ,
}
...
init_global_animations::proc(){
	for a_name in Animation_Name {
		g_mem.assets.animations.global[a_name] = animation_create(a_name)
	}	
}
update_global_animations::proc(){
	for a_name in Animation_Name {
		animation_update(&g_mem.assets.animations.global[a_name],rl.GetFrameTime())
	}	
}